...
import org.bem.iot.media.callback.MKSourceFindCallBack;
import org.bem.iot.media.config.MediaServerConfig;
import org.bem.iot.media.constants.MediaServerConstants;
import org.bem.iot.media.context.MediaServerContext;
import org.bem.iot.media.model.param.*;
import org.bem.iot.media.model.result.*;
...
public class ApiService {
    ...
    public StreamUrlResult addStreamProxy(StreamProxyParam param) {
        ...
        IMKProxyPlayerCallBack imkProxyPlayerCallBack = (pUser, err, what, sys_err) -> {
            if (err != 0) {
                logger.warn("【MediaServer】拉流代理失败：{}", what);
                queue.offer(what);
            } else {
                queue.offer(key);
                logger.info("【MediaServer】拉流代理成功");
            }
        };
        ...
        IMKProxyPlayerCallBack imkProxyPlayCloseCallBack = (pUser, err, what, sys_err) -> {
            ...
            logger.info("【MediaServer】拉流代理关闭");
    ...
    public String addStreamPusherProxy(StreamPushProxyParam param) {
        ...
        ZLM_API.mk_pusher_set_on_result(mkPusher, (user_data, err_code, err_msg) -> {
            if (err_code != 0) {
                queue.offer(err_msg);
                logger.warn("【MediaServer】推流代理失败：{}", err_msg);
            } else {
                queue.offer(key);
                logger.info("【MediaServer】推流代理成功");
            }
        }, mkPusher.getPointer());
        IMKPushEventCallBack imkPushEventCallBack = (user_data, err_code, err_msg) -> {
            ...
            logger.info("【MediaServer】推流代理关闭");
        };
    ...
    public Boolean startRecord(StartRecordParam param) {
        int ret = ZLM_API.mk_recorder_start(param.getType(), MediaServerConstants.DEFAULT_VHOST, param.getApp(), param.getStream(), param.getCustomizedPath(), param.getMaxSecond());
        return ret == 1;
    }
    ...
    public Integer openRtpServer(OpenRtpServerParam param) {
        ...
        MK_RTP_SERVER mkRtpServer = ZLM_API.mk_rtp_server_create(port, param.getTcpMode(), param.getStream());
        if (mkRtpServer == null) {
            return -1;
        }
